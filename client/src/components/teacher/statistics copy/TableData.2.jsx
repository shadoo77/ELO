...
// Chart bar
import ProgressBar from "./ProgressBar.2";
// Material ui
import { Grid, Tooltip } from "@material-ui/core/";
import { makeStyles } from "@material-ui/core/styles";
// Helpers
import { getLastNodeOfTree } from "services/searchInTree";
...
function TableData(props) {
  ...
  // const allParaOfBranchId = getLastNodeOfTree(tree, branchId);
  // console.log("getAllParaOfBranchId ::: ", allParaOfBranchId);
  ...
  const renderProgressBarForEachItem = slideshows => {
    if (children && children.length) {
      return children.map((child, i) => {
          ...
          paragrafs.forEach(paragraf => {
            if (slideshows && slideshows.length) {
              slideshows.forEach(el => {
                if (el.entrypoint === paragraf.id) {
                  slideshowOf.push(el);
                }
              });
              slides = extractAllSlides(slideshowOf);
              temp = handleSlideshowsInteractions(slideshowOf);
            }
          });
        } else if (child.depthLevel === "paragraf") {
          if (slideshows && slideshows.length) {
            slideshows.forEach(el => {
              if (el.entrypoint === child.id) {
                slideshowOf.push(el);
              }
            });
            slides = extractAllSlides(slideshowOf);
        ...
        // const progress = temp.progress;
        // const correctPrecent = temp.correct;
        // const wronfPrecent = temp.wrong;
        ...
        //console.log("slideshowOf >>>>>>>>>> ", slideshowOf);
        ...
        return (
          <Grid item className={classes.cellStyle} key={"heeey" + i}>
            <Link
              to={`${routeUrls.teacher.group.statusTest}/${groupId}/depth/${child.depthLevel}/branch/${child.id}/student/${studentId}`}
            >
              <Tooltip title={child.value}>
                <div>
                  <ProgressBarItem
                    ...
                    // allQuestions={true}
                    // progress={progress}
                    // correctPrecent={correctPrecent}
                    // wronfPrecent={wronfPrecent}
                  />
                </div>
              </Tooltip>
            </Link>
