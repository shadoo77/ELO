...
export default props => {
  const { category, paragraph } = props;
    ...
    <Box width={8 / 10} mx={"auto"} mt={2}>
      <Paper>
        <List component="nav" className={classes.root}>
          <ListItem>
            <ListItemIcon>{categoryIcon(category.type)}</ListItemIcon>
            <ListItemText
              primary={paragraph + " " + categoryName(category.type)}
            />
          </ListItem>
          {category.items && category.items.length ? (
            category.items.map((item, i) => (
              <React.Fragment key={item._id}>
                ...
                {i === category.items.length - 1 ? null : <Divider />}
              </React.Fragment>
            ))
          ) : (
